---
title: Dreamer - Flexibility (Flaw)
formula: 1d6
aliases:
  - Dreamer - Flexibility
foundry:
  base: RollTable
  id: SZTZYLCrhz5Cc0Ao
  data:
    formula: 1d6
    img: icons/magic/time/day-night-sunset-sunrise.webp
    results:
      - range:
          - 1
          - 2
        text: 'Rigid: I can''t keep a secret to save my life, or anyone else''s.'
        weight: 2
      - range:
          - 3
          - 4
        text: 'Slightly Yielding: I am dogmatic in my thoughts and philosophy.'
        weight: 2
      - range:
          - 5
          - 6
        text: 'Surprisingly Swishy: I am easily distracted and if there''s a plan, I''ll forget it. If I don''t forget it, I''ll ignore it.'
        weight: 2
  folder: Dreamer
---

```rolltable
foundry.data
```


